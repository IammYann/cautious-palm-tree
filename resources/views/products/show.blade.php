...
<div style="background: white; padding: 2rem; border-radius: 8px;">
    ...
    {{-- FonePay Purchase Section --}}
    <div style="background: linear-gradient(135deg, #fce4ec, #ffcdd2); border: 2px solid #e1232b; border-radius: 12px; padding: 2rem; margin-bottom: 2rem;">
        <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1rem;">
            <div>
                <span style="font-size: 1.5rem; font-weight: 700; color: #e1232b;">fonepay</span>
            </div>
            <h3 style="color: #2c3e50; margin: 0;">Pay with FonePay</h3>
        </div>
        ...
        @else
            @auth
                <form action="{{ route('payment.fonepay.initiate', $product->id) }}" method="POST" style="background: none; box-shadow: none; padding: 0; margin: 0;">
                    @csrf
                    <div style="display: flex; align-items: center; gap: 1rem; flex-wrap: wrap;">
                        <button type="submit" id="fonepay-buy-btn" style="
                            background: linear-gradient(135deg, #e1232b, #c41e24);
                            color: white;
                            border: none;
                            padding: 0.75rem 2rem;
                            border-radius: 8px;
                            ...
                            cursor: pointer;
                            transition: all 0.3s ease;
                            box-shadow: 0 4px 15px rgba(225, 35, 43, 0.3);
                            display: flex;
                            align-items: center;
                            gap: 0.5rem;
                        " 
                        onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 6px 20px rgba(225, 35, 43, 0.4)'; this.style.background='linear-gradient(135deg, #c41e24, #a5181d)';"
                           onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 15px rgba(225, 35, 43, 0.3)'; this.style.background='linear-gradient(135deg, #e1232b, #c41e24)';"
                        >
                        Buy with FonePay — Rs. {{ number_format($product->price, 2) }}
                        </button>
                    </div>
                </form>
            @else
